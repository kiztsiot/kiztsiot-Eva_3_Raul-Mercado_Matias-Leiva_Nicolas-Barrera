...
<div class="flex items-center justify-center min-h-[80vh] px-4">
    <div class="w-full max-w-xl bg-white rounded-2xl shadow-xl border border-gray-100 p-8 space-y-6">
        ...
        <div class="text-center space-y-2">
            ...
            <h2 class="text-2xl font-bold tracking-tight text-gray-900">¡Bienvenido, {{ $usuario->nombre }}!</h2>
        ...
        <div class="space-y-2">
            <x-atoms.label class="text-xs font-semibold text-gray-600 uppercase tracking-wider">Tu token JWT:</x-atoms.label>
            <div class="bg-gray-900 text-gray-100 p-4 rounded-xl overflow-x-auto shadow-inner border border-gray-800">
                <pre class="text-xs font-mono break-all whitespace-pre-wrap">{{ $token }}</pre>
            </div>
        ...
        <div class="pt-4 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-3">
            ...
            <a href="{{ route('proyectos') }}" class="w-full sm:w-auto inline-flex justify-center items-center py-2 px-4 border border-transparent rounded-xl shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors">
